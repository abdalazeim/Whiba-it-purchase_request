...
class PurchaseRequest(models.Model):
    ...
    assigned_to = fields.Many2one(
        ...
        domain=lambda self: [
            (
                "groups_id",
                "in",
                self.env.ref("Whiba-it-purchase_request.group_purchase_request_manager").id,
            )
        ],
        index=True,
    )
    ...
    def action_view_purchase_request_line(self):
        action = (
            self.env.ref("Whiba-it-purchase_request.purchase_request_line_form_action")
            .sudo()
            .read()[0]
        )
        ...
        if len(lines) > 1:
            ...
            action["views"] = [
                (self.env.ref("Whiba-it-purchase_request.purchase_request_line_form").id, "form")
            ]
